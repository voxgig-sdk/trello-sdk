...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Board()` — with named operations (`list`/`load`/`create`/`update`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
try {
    // list() returns an array of Board records — iterate directly.
    $boards = $client->Board()->list();
    foreach ($boards as $item) {
        echo $item["id"] . " " . $item["closed"] . "\n";
    }
} catch (\Throwable $err) {
...
```php
// create() returns the ENTITY — call data_get() for the created Board record.
$created = $client->Board()->create(["id" => "example_id"]);

// Update — index the record via data_get() ($created->data_get()["id"]).
$client->Board()->update(["id" => $created->data_get()["id"], "closed" => true, "creationMethod" => "example_creationMethod"]);

// Remove
$client->Board()->remove(["id" => $created->data_get()["id"]]);
```
...
// $board->data_get() now returns the board data from the last list
// $board->match_get() returns the last match criteria
```
