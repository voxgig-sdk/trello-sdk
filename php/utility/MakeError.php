...
class TrelloMakeError
{
    public static function call(?TrelloContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TrelloContext([], null);
        }
        $op = $ctx->op ?? new TrelloOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TrelloResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TrelloError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TrelloError('', $msg, $ctx);
