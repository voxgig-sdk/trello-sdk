...
// Trello SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TrelloOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TrelloOmni::structfix($now, Struct::undef()))
                        !== json_encode(TrelloOmni::structfix($val, Struct::undef()))) {
