...
class PluginEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Plugin(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Plugin(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_PLUGIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $plugin_ref01_list_result = $plugin_ref01_ent->list($plugin_ref01_match, null);
        $this->assertIsArray($plugin_ref01_list_result);

        // UPDATE
        $plugin_ref01_data_up0_up = [
            "id" => $plugin_ref01_data["id"],
        ];

        $plugin_ref01_resdata_up0_result = $plugin_ref01_ent->update($plugin_ref01_data_up0_up, null);
        $plugin_ref01_resdata_up0 = Helpers::to_map(is_object($plugin_ref01_resdata_up0_result) && method_exists($plugin_ref01_resdata_up0_result, 'data_get') ? $plugin_ref01_resdata_up0_result->data_get() : $plugin_ref01_resdata_up0_result);
        $this->assertNotNull($plugin_ref01_resdata_up0);
        $this->assertEquals($plugin_ref01_resdata_up0["id"], $plugin_ref01_data_up0_up["id"]);
...
function plugin_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["plugin01", "plugin02", "plugin03", "board01", "board02", "board03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_PLUGIN_ENTID");
