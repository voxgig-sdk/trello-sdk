...
class WebhookEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_WEBHOOK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function webhook_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["webhook01", "webhook02", "webhook03", "token01", "token02", "token03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_WEBHOOK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["TRELLO_TEST_WEBHOOK_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }
    if (!isset($idmap_resolved["token_id"])) {
        $idmap_resolved["token_id"] = $idmap_resolved["token01"];
    }
