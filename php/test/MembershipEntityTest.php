...
class MembershipEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_MEMBERSHIP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function membership_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("TRELLO_TEST_MEMBERSHIP_ENTID");
    ...
    if (!isset($idmap_resolved["board_id"])) {
