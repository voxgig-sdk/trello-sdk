...
class CardListEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_CARD_LIST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $card_list_ref01_data = null;
        if (count($card_list_ref01_data_raw) > 0) {
...
function card_list_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("TRELLO_TEST_CARD_LIST_ENTID");
