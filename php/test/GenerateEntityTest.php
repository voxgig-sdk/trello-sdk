...
class GenerateEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_GENERATE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function generate_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["generate01", "generate02", "generate03", "board01", "board02", "board03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_GENERATE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["TRELLO_TEST_GENERATE_ENTID"]);
