...
class MembersVotedEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TrelloSDK::test(null, null);
        $ent = $testsdk->MembersVoted(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = members_voted_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "members_voted." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_MEMBERS_VOTED_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $members_voted_ref01_data = null;
        if (count($members_voted_ref01_data_raw) > 0) {
...
function members_voted_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/members_voted/MembersVotedTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["members_voted01", "members_voted02", "members_voted03", "card01", "card02", "card03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_MEMBERS_VOTED_ENTID");
