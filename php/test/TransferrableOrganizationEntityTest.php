...
class TransferrableOrganizationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TrelloSDK::test(null, null);
        $ent = $testsdk->TransferrableOrganization(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = transferrable_organization_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "transferrable_organization." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_TRANSFERRABLE_ORGANIZATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $transferrable_organization_ref01_data = null;
        if (count($transferrable_organization_ref01_data_raw) > 0) {
...
function transferrable_organization_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/transferrable_organization/TransferrableOrganizationTestData.json';
    ...
    $entid_env_raw = getenv("TRELLO_TEST_TRANSFERRABLE_ORGANIZATION_ENTID");
