...
class AttachmentEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_ATTACHMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $attachment_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.attachment")));
        $attachment_ref01_data = null;
        if (count($attachment_ref01_data_raw) > 0) {
            $attachment_ref01_data = Helpers::to_map($attachment_ref01_data_raw[0][1]);
        }
...
function attachment_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["attachment01", "attachment02", "attachment03", "card01", "card02", "card03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_ATTACHMENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["TRELLO_TEST_ATTACHMENT_ENTID"]);
