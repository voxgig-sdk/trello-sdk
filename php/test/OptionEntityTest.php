...
class OptionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_OPTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function option_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["option01", "option02", "option03", "custom_field01", "custom_field02", "custom_field03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_OPTION_ENTID");
