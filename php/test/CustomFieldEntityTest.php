...
class CustomFieldEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->CustomField(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->CustomField(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = custom_field_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "custom_field." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_CUSTOM_FIELD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $custom_field_ref01_ent = $client->CustomField(null);
        ...
        $custom_field_ref01_data_result = $custom_field_ref01_ent->create($custom_field_ref01_data, null);
        $custom_field_ref01_data = Helpers::to_map(is_object($custom_field_ref01_data_result) && method_exists($custom_field_ref01_data_result, 'data_get') ? $custom_field_ref01_data_result->data_get() : $custom_field_ref01_data_result);
        $this->assertNotNull($custom_field_ref01_data);
        $this->assertNotNull($custom_field_ref01_data["id"]);
        ...
        $custom_field_ref01_list_result = $custom_field_ref01_ent->list($custom_field_ref01_match, null);
        $this->assertIsArray($custom_field_ref01_list_result);
        ...
        $custom_field_ref01_ent->remove($custom_field_ref01_match_rm0, null);
        ...
        $custom_field_ref01_list_rt0_result = $custom_field_ref01_ent->list($custom_field_ref01_match_rt0, null);
        $this->assertIsArray($custom_field_ref01_list_rt0_result);
...
function custom_field_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/custom_field/CustomFieldTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["custom_field01", "custom_field02", "custom_field03", "board01", "board02", "board03", "card01", "card02", "card03", "id_card01"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_CUSTOM_FIELD_ENTID");
