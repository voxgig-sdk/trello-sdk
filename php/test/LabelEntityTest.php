...
class LabelEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_LABEL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function label_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["label01", "label02", "label03", "board01", "board02", "board03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_LABEL_ENTID");
