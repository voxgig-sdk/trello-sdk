...
class StickerEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TrelloSDK::test(null, null);
        $ent = $testsdk->Sticker(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = sticker_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "sticker." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_STICKER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $sticker_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.sticker")));
        $sticker_ref01_data = null;
        if (count($sticker_ref01_data_raw) > 0) {
            $sticker_ref01_data = Helpers::to_map($sticker_ref01_data_raw[0][1]);
        }
...
function sticker_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/sticker/StickerTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["sticker01", "sticker02", "sticker03", "card01", "card02", "card03"] as $k) {
    ...
    $entid_env_raw = getenv("TRELLO_TEST_STICKER_ENTID");
    ...
    if (!isset($idmap_resolved["card_id"])) {
