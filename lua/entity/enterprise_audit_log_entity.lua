...
local EnterpriseAuditLogEntity = {}
EnterpriseAuditLogEntity.__index = EnterpriseAuditLogEntity


function EnterpriseAuditLogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EnterpriseAuditLogEntity)
  self._name = "enterprise_audit_log"
  self._client = client
...
function EnterpriseAuditLogEntity:get_name()
  return self._name
end


function EnterpriseAuditLogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EnterpriseAuditLogEntity.new(self._client, opts)
end
...
function EnterpriseAuditLogEntity:mark_deleted()
  self._deleted = true
end


function EnterpriseAuditLogEntity:deleted()
  return true == self._deleted
end


function EnterpriseAuditLogEntity:data_set(args)
  if args ~= nil then
...
function EnterpriseAuditLogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EnterpriseAuditLogEntity:match_set(args)
  if args ~= nil then
...
function EnterpriseAuditLogEntity:match_get()
...
function EnterpriseAuditLogEntity:stream(action, args, callopts)
...
---@param reqmatch EnterpriseAuditLogListMatch
---@param ctrl? table
---@return EnterpriseAuditLog[]
---@return string? err
function EnterpriseAuditLogEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function EnterpriseAuditLogEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return EnterpriseAuditLogEntity
