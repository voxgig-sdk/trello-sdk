...
AttachmentEntity.__index = AttachmentEntity


function AttachmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AttachmentEntity)
  self._name = "attachment"
  self._client = client
...
function AttachmentEntity:get_name()
  return self._name
end


function AttachmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AttachmentEntity.new(self._client, opts)
end
...
function AttachmentEntity:mark_deleted()
  self._deleted = true
end


function AttachmentEntity:deleted()
  return true == self._deleted
end


function AttachmentEntity:data_set(args)
  if args ~= nil then
...
function AttachmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AttachmentEntity:match_set(args)
  if args ~= nil then
...
function AttachmentEntity:match_get()
...
function AttachmentEntity:stream(action, args, callopts)
...
---@param reqmatch AttachmentLoadMatch
---@param ctrl? table
---@return Attachment
---@return string? err
function AttachmentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch AttachmentListMatch
---@param ctrl? table
---@return Attachment[]
---@return string? err
function AttachmentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqmatch AttachmentRemoveMatch
---@param ctrl? table
---@return Attachment
---@return string? err
function AttachmentEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function AttachmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
