...
local ExportDownloadEntity = {}
ExportDownloadEntity.__index = ExportDownloadEntity


function ExportDownloadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ExportDownloadEntity)
  self._name = "export_download"
  self._client = client
...
function ExportDownloadEntity:get_name()
  return self._name
end


function ExportDownloadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ExportDownloadEntity.new(self._client, opts)
end
...
function ExportDownloadEntity:mark_deleted()
  self._deleted = true
end


function ExportDownloadEntity:deleted()
  return true == self._deleted
end


function ExportDownloadEntity:data_set(args)
  if args ~= nil then
...
function ExportDownloadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ExportDownloadEntity:match_set(args)
  if args ~= nil then
...
function ExportDownloadEntity:match_get()
...
function ExportDownloadEntity:stream(action, args, callopts)
...
---@param reqmatch ExportDownloadLoadMatch
---@param ctrl? table
---@return ExportDownload
---@return string? err
function ExportDownloadEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ExportDownloadEntity:_run_op(ctx, post_done)
  local utility = self._utility
