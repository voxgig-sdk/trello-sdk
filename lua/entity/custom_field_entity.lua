...
local CustomFieldEntity = {}
CustomFieldEntity.__index = CustomFieldEntity


function CustomFieldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomFieldEntity)
  self._name = "custom_field"
  self._client = client
...
function CustomFieldEntity:get_name()
  return self._name
end


function CustomFieldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomFieldEntity.new(self._client, opts)
end
...
function CustomFieldEntity:mark_deleted()
  self._deleted = true
end


function CustomFieldEntity:deleted()
  return true == self._deleted
end


function CustomFieldEntity:data_set(args)
  if args ~= nil then
...
function CustomFieldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomFieldEntity:match_set(args)
  if args ~= nil then
...
function CustomFieldEntity:match_get()
...
function CustomFieldEntity:stream(action, args, callopts)
...
---@param reqmatch CustomFieldLoadMatch
---@param ctrl? table
---@return CustomField
---@return string? err
function CustomFieldEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CustomFieldListMatch
---@param ctrl? table
---@return CustomField[]
---@return string? err
function CustomFieldEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CustomFieldCreateData
---@param ctrl? table
---@return CustomField
---@return string? err
function CustomFieldEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata CustomFieldUpdateData
---@param ctrl? table
---@return CustomField
---@return string? err
function CustomFieldEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch CustomFieldRemoveMatch
---@param ctrl? table
---@return CustomField
---@return string? err
function CustomFieldEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function CustomFieldEntity:_run_op(ctx, post_done)
  local utility = self._utility
