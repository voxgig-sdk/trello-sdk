...
local PluginDataEntity = {}
PluginDataEntity.__index = PluginDataEntity


function PluginDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PluginDataEntity)
  self._name = "plugin_data"
  self._client = client
...
function PluginDataEntity:get_name()
  return self._name
end


function PluginDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PluginDataEntity.new(self._client, opts)
end
...
function PluginDataEntity:mark_deleted()
  self._deleted = true
end


function PluginDataEntity:deleted()
  return true == self._deleted
end


function PluginDataEntity:data_set(args)
  if args ~= nil then
...
function PluginDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PluginDataEntity:match_set(args)
  if args ~= nil then
...
function PluginDataEntity:match_get()
...
function PluginDataEntity:stream(action, args, callopts)
...
---@param reqmatch PluginDataLoadMatch
---@param ctrl? table
---@return PluginData
---@return string? err
function PluginDataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PluginDataListMatch
---@param ctrl? table
---@return PluginData[]
---@return string? err
function PluginDataEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PluginDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
