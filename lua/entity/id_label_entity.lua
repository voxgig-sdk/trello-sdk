...
local IdLabelEntity = {}
IdLabelEntity.__index = IdLabelEntity


function IdLabelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IdLabelEntity)
  self._name = "id_label"
  self._client = client
...
function IdLabelEntity:get_name()
  return self._name
end


function IdLabelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IdLabelEntity.new(self._client, opts)
end
...
function IdLabelEntity:mark_deleted()
  self._deleted = true
end


function IdLabelEntity:deleted()
  return true == self._deleted
end


function IdLabelEntity:data_set(args)
  if args ~= nil then
...
function IdLabelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IdLabelEntity:match_set(args)
  if args ~= nil then
...
function IdLabelEntity:match_get()
...
function IdLabelEntity:stream(action, args, callopts)
...
---@param reqmatch IdLabelRemoveMatch
---@param ctrl? table
---@return IdLabel
---@return string? err
function IdLabelEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function IdLabelEntity:_run_op(ctx, post_done)
  local utility = self._utility
