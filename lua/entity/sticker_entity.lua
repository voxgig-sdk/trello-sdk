...
local StickerEntity = {}
StickerEntity.__index = StickerEntity


function StickerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StickerEntity)
  self._name = "sticker"
  self._client = client
...
function StickerEntity:get_name()
  return self._name
end


function StickerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StickerEntity.new(self._client, opts)
end
...
function StickerEntity:mark_deleted()
  self._deleted = true
end


function StickerEntity:deleted()
  return true == self._deleted
end


function StickerEntity:data_set(args)
  if args ~= nil then
...
function StickerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StickerEntity:match_set(args)
  if args ~= nil then
...
function StickerEntity:match_get()
...
function StickerEntity:stream(action, args, callopts)
...
---@param reqmatch StickerLoadMatch
---@param ctrl? table
---@return Sticker
---@return string? err
function StickerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata StickerUpdateData
---@param ctrl? table
---@return Sticker
---@return string? err
function StickerEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch StickerRemoveMatch
---@param ctrl? table
---@return Sticker
---@return string? err
function StickerEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function StickerEntity:_run_op(ctx, post_done)
  local utility = self._utility
