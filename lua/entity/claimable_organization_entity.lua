-- Trello SDK ClaimableOrganization entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ClaimableOrganizationEntity = {}
ClaimableOrganizationEntity.__index = ClaimableOrganizationEntity


function ClaimableOrganizationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClaimableOrganizationEntity)
  self._name = "claimable_organization"
  self._client = client
...
function ClaimableOrganizationEntity:get_name()
  return self._name
end


function ClaimableOrganizationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClaimableOrganizationEntity.new(self._client, opts)
end
...
function ClaimableOrganizationEntity:mark_deleted()
  self._deleted = true
end


function ClaimableOrganizationEntity:deleted()
  return true == self._deleted
end


function ClaimableOrganizationEntity:data_set(args)
  if args ~= nil then
...
function ClaimableOrganizationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClaimableOrganizationEntity:match_set(args)
  if args ~= nil then
...
function ClaimableOrganizationEntity:match_get()
...
function ClaimableOrganizationEntity:stream(action, args, callopts)
...
---@param reqmatch ClaimableOrganizationListMatch
---@param ctrl? table
---@return ClaimableOrganization[]
---@return string? err
function ClaimableOrganizationEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ClaimableOrganizationEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ClaimableOrganizationEntity
