...
ChecklistEntity.__index = ChecklistEntity


function ChecklistEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ChecklistEntity)
  self._name = "checklist"
  self._client = client
...
function ChecklistEntity:get_name()
  return self._name
end


function ChecklistEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ChecklistEntity.new(self._client, opts)
end
...
function ChecklistEntity:mark_deleted()
  self._deleted = true
end


function ChecklistEntity:deleted()
  return true == self._deleted
end


function ChecklistEntity:data_set(args)
  if args ~= nil then
...
function ChecklistEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ChecklistEntity:match_set(args)
  if args ~= nil then
...
function ChecklistEntity:match_get()
...
function ChecklistEntity:stream(action, args, callopts)
...
---@param reqmatch ChecklistLoadMatch
---@param ctrl? table
---@return Checklist
---@return string? err
function ChecklistEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ChecklistCreateData
---@param ctrl? table
---@return Checklist
---@return string? err
function ChecklistEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata ChecklistUpdateData
---@param ctrl? table
---@return Checklist
---@return string? err
function ChecklistEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch ChecklistRemoveMatch
---@param ctrl? table
---@return Checklist
---@return string? err
function ChecklistEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function ChecklistEntity:_run_op(ctx, post_done)
  local utility = self._utility
