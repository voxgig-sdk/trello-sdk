...
local ReadEntity = {}
ReadEntity.__index = ReadEntity


function ReadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReadEntity)
  self._name = "read"
  self._client = client
...
function ReadEntity:get_name()
  return self._name
end


function ReadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReadEntity.new(self._client, opts)
end
...
function ReadEntity:mark_deleted()
  self._deleted = true
end


function ReadEntity:deleted()
  return true == self._deleted
end


function ReadEntity:data_set(args)
  if args ~= nil then
...
function ReadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReadEntity:match_set(args)
  if args ~= nil then
...
function ReadEntity:stream(action, args, callopts)
...
---@param reqdata ReadCreateData
---@param ctrl? table
---@return Read
---@return string? err
function ReadEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ReadEntity:_run_op(ctx, post_done)
  local utility = self._utility
