...
local CustomStickerEntity = {}
CustomStickerEntity.__index = CustomStickerEntity


function CustomStickerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomStickerEntity)
  self._name = "custom_sticker"
  self._client = client
...
function CustomStickerEntity:get_name()
  return self._name
end


function CustomStickerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomStickerEntity.new(self._client, opts)
end
...
function CustomStickerEntity:mark_deleted()
  self._deleted = true
end


function CustomStickerEntity:deleted()
  return true == self._deleted
end


function CustomStickerEntity:data_set(args)
  if args ~= nil then
...
function CustomStickerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomStickerEntity:match_set(args)
  if args ~= nil then
...
function CustomStickerEntity:match_get()
...
function CustomStickerEntity:stream(action, args, callopts)
...
---@param reqmatch CustomStickerLoadMatch
---@param ctrl? table
---@return CustomSticker
---@return string? err
function CustomStickerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CustomStickerListMatch
---@param ctrl? table
---@return CustomSticker[]
---@return string? err
function CustomStickerEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CustomStickerCreateData
---@param ctrl? table
---@return CustomSticker
---@return string? err
function CustomStickerEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch CustomStickerRemoveMatch
---@param ctrl? table
---@return CustomSticker
---@return string? err
function CustomStickerEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function CustomStickerEntity:_run_op(ctx, post_done)
  local utility = self._utility
