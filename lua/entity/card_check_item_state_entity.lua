...
local CardCheckItemStateEntity = {}
CardCheckItemStateEntity.__index = CardCheckItemStateEntity


function CardCheckItemStateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CardCheckItemStateEntity)
  self._name = "card_check_item_state"
  self._client = client
...
function CardCheckItemStateEntity:get_name()
  return self._name
end


function CardCheckItemStateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CardCheckItemStateEntity.new(self._client, opts)
end
...
function CardCheckItemStateEntity:mark_deleted()
  self._deleted = true
end


function CardCheckItemStateEntity:deleted()
  return true == self._deleted
end


function CardCheckItemStateEntity:data_set(args)
  if args ~= nil then
...
function CardCheckItemStateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CardCheckItemStateEntity:match_set(args)
  if args ~= nil then
...
function CardCheckItemStateEntity:match_get()
...
function CardCheckItemStateEntity:stream(action, args, callopts)
...
---@param reqmatch CardCheckItemStateLoadMatch
---@param ctrl? table
---@return CardCheckItemState
---@return string? err
function CardCheckItemStateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CardCheckItemStateEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CardCheckItemStateEntity
