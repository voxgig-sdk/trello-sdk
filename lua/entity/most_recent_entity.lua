-- Trello SDK MostRecent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MostRecentEntity = {}
MostRecentEntity.__index = MostRecentEntity


function MostRecentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MostRecentEntity)
  self._name = "most_recent"
  self._client = client
...
function MostRecentEntity:get_name()
  return self._name
end


function MostRecentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MostRecentEntity.new(self._client, opts)
end
...
function MostRecentEntity:mark_deleted()
  self._deleted = true
end


function MostRecentEntity:deleted()
  return true == self._deleted
end


function MostRecentEntity:data_set(args)
  if args ~= nil then
...
function MostRecentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MostRecentEntity:match_set(args)
  if args ~= nil then
...
function MostRecentEntity:match_get()
...
function MostRecentEntity:stream(action, args, callopts)
...
function MostRecentEntity:_run_op(ctx, post_done)
  local utility = self._utility
