...
local NotificationListEntity = {}
NotificationListEntity.__index = NotificationListEntity


function NotificationListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NotificationListEntity)
  self._name = "notification_list"
  self._client = client
...
function NotificationListEntity:get_name()
  return self._name
end


function NotificationListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NotificationListEntity.new(self._client, opts)
end
...
function NotificationListEntity:mark_deleted()
  self._deleted = true
end


function NotificationListEntity:deleted()
  return true == self._deleted
end


function NotificationListEntity:data_set(args)
  if args ~= nil then
...
function NotificationListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NotificationListEntity:match_set(args)
  if args ~= nil then
...
function NotificationListEntity:match_get()
...
function NotificationListEntity:stream(action, args, callopts)
...
---@param reqmatch NotificationListLoadMatch
---@param ctrl? table
---@return NotificationList
---@return string? err
function NotificationListEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NotificationListEntity:_run_op(ctx, post_done)
  local utility = self._utility
