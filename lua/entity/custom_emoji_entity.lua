...
local CustomEmojiEntity = {}
CustomEmojiEntity.__index = CustomEmojiEntity


function CustomEmojiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomEmojiEntity)
  self._name = "custom_emoji"
  self._client = client
...
function CustomEmojiEntity:get_name()
  return self._name
end


function CustomEmojiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomEmojiEntity.new(self._client, opts)
end
...
function CustomEmojiEntity:mark_deleted()
  self._deleted = true
end


function CustomEmojiEntity:deleted()
  return true == self._deleted
end


function CustomEmojiEntity:data_set(args)
  if args ~= nil then
...
function CustomEmojiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomEmojiEntity:match_set(args)
  if args ~= nil then
...
function CustomEmojiEntity:match_get()
...
function CustomEmojiEntity:stream(action, args, callopts)
...
---@param reqmatch CustomEmojiLoadMatch
---@param ctrl? table
---@return CustomEmoji
---@return string? err
function CustomEmojiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CustomEmojiListMatch
---@param ctrl? table
---@return CustomEmoji[]
---@return string? err
function CustomEmojiEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CustomEmojiCreateData
---@param ctrl? table
---@return CustomEmoji
---@return string? err
function CustomEmojiEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CustomEmojiEntity:_run_op(ctx, post_done)
  local utility = self._utility
