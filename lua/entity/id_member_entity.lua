...
local IdMemberEntity = {}
IdMemberEntity.__index = IdMemberEntity


function IdMemberEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IdMemberEntity)
  self._name = "id_member"
  self._client = client
...
function IdMemberEntity:get_name()
  return self._name
end


function IdMemberEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IdMemberEntity.new(self._client, opts)
end
...
function IdMemberEntity:mark_deleted()
  self._deleted = true
end


function IdMemberEntity:deleted()
  return true == self._deleted
end


function IdMemberEntity:data_set(args)
  if args ~= nil then
...
function IdMemberEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IdMemberEntity:match_set(args)
  if args ~= nil then
...
function IdMemberEntity:match_get()
...
function IdMemberEntity:stream(action, args, callopts)
...
---@param reqmatch IdMemberRemoveMatch
---@param ctrl? table
---@return IdMember
---@return string? err
function IdMemberEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function IdMemberEntity:_run_op(ctx, post_done)
  local utility = self._utility
