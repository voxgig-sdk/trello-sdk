...
local EmailPositionEntity = {}
EmailPositionEntity.__index = EmailPositionEntity


function EmailPositionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmailPositionEntity)
  self._name = "email_position"
  self._client = client
...
function EmailPositionEntity:get_name()
  return self._name
end


function EmailPositionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmailPositionEntity.new(self._client, opts)
end
...
function EmailPositionEntity:mark_deleted()
  self._deleted = true
end


function EmailPositionEntity:deleted()
  return true == self._deleted
end


function EmailPositionEntity:data_set(args)
  if args ~= nil then
...
function EmailPositionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmailPositionEntity:match_set(args)
  if args ~= nil then
...
function EmailPositionEntity:match_get()
...
function EmailPositionEntity:stream(action, args, callopts)
...
---@param reqdata EmailPositionUpdateData
---@param ctrl? table
---@return EmailPosition
---@return string? err
function EmailPositionEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function EmailPositionEntity:_run_op(ctx, post_done)
  local utility = self._utility
