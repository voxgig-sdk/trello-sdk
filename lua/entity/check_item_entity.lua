...
local CheckItemEntity = {}
CheckItemEntity.__index = CheckItemEntity


function CheckItemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CheckItemEntity)
  self._name = "check_item"
  self._client = client
...
function CheckItemEntity:get_name()
  return self._name
end


function CheckItemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CheckItemEntity.new(self._client, opts)
end
...
function CheckItemEntity:mark_deleted()
  self._deleted = true
end


function CheckItemEntity:deleted()
  return true == self._deleted
end


function CheckItemEntity:data_set(args)
  if args ~= nil then
...
function CheckItemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CheckItemEntity:match_set(args)
  if args ~= nil then
...
function CheckItemEntity:match_get()
...
function CheckItemEntity:stream(action, args, callopts)
...
---@param reqmatch CheckItemLoadMatch
---@param ctrl? table
---@return CheckItem
---@return string? err
function CheckItemEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CheckItemUpdateData
---@param ctrl? table
---@return CheckItem
---@return string? err
function CheckItemEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch CheckItemRemoveMatch
---@param ctrl? table
---@return CheckItem
---@return string? err
function CheckItemEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function CheckItemEntity:_run_op(ctx, post_done)
  local utility = self._utility
