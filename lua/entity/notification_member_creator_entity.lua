-- Trello SDK NotificationMemberCreator entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local NotificationMemberCreatorEntity = {}
NotificationMemberCreatorEntity.__index = NotificationMemberCreatorEntity


function NotificationMemberCreatorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NotificationMemberCreatorEntity)
  self._name = "notification_member_creator"
  self._client = client
...
function NotificationMemberCreatorEntity:get_name()
  return self._name
end


function NotificationMemberCreatorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NotificationMemberCreatorEntity.new(self._client, opts)
end
...
function NotificationMemberCreatorEntity:mark_deleted()
  self._deleted = true
end


function NotificationMemberCreatorEntity:deleted()
  return true == self._deleted
end


function NotificationMemberCreatorEntity:data_set(args)
  if args ~= nil then
...
function NotificationMemberCreatorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NotificationMemberCreatorEntity:match_set(args)
  if args ~= nil then
...
function NotificationMemberCreatorEntity:match_get()
...
function NotificationMemberCreatorEntity:stream(action, args, callopts)
...
---@param reqmatch NotificationMemberCreatorLoadMatch
---@param ctrl? table
---@return NotificationMemberCreator
---@return string? err
function NotificationMemberCreatorEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NotificationMemberCreatorEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return NotificationMemberCreatorEntity
