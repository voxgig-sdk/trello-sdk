...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Board()` — each with the same small set of operations (`list`, `load`, `create`, `update`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
for _, item in ipairs(boards) do
  print(item["id"], item["creationMethod"])
end
```
...
```lua
-- Create
local created, err = client:Board():create({ id = "example_id" })
if err then error(err) end
...
local result, err = client:Board():list()
