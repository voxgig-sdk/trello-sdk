...
describe("SavedSearchDirect", function()
  it("should direct-list-saved_search", function()
    local setup = saved_search_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-saved_search", setup.live and "live" or "unit")
  ...
  it("should direct-load-saved_search", function()
    local setup = saved_search_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-saved_search", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function saved_search_direct_setup(mockres)
