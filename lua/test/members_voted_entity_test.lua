...
describe("MembersVotedEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:MembersVoted(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = members_voted_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "members_voted." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_MEMBERS_VOTED_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local members_voted_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.members_voted")))
    local members_voted_ref01_data = nil
    if #members_voted_ref01_data_raw > 0 then
      members_voted_ref01_data = helpers.to_map(members_voted_ref01_data_raw[1][2])
    end
...
function members_voted_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/members_voted/MembersVotedTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read members_voted test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("TRELLO_TEST_MEMBERS_VOTED_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["TRELLO_TEST_MEMBERS_VOTED_ENTID"])
