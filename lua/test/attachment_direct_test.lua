...
describe("AttachmentDirect", function()
  it("should direct-list-attachment", function()
    local setup = attachment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-attachment", setup.live and "live" or "unit")
  ...
  it("should direct-load-attachment", function()
    local setup = attachment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-attachment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function attachment_direct_setup(mockres)
