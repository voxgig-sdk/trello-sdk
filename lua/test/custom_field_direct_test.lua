...
describe("CustomFieldDirect", function()
  it("should direct-list-custom_field", function()
    local setup = custom_field_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-custom_field", setup.live and "live" or "unit")
  ...
  it("should direct-load-custom_field", function()
    local setup = custom_field_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-custom_field", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function custom_field_direct_setup(mockres)
