...
describe("CardEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_CARD_ENTID JSON to run live")
      return
    end
    ...
    -- UPDATE
    local card_ref01_data_up0_up = {
      id = card_ref01_data["id"],
    }

    local card_ref01_markdef_up0_name = "address"
    local card_ref01_markdef_up0_value = "Mark01-card_ref01_" .. tostring(setup.now)
    card_ref01_data_up0_up[card_ref01_markdef_up0_name] = card_ref01_markdef_up0_value

    local card_ref01_resdata_up0_result, err = card_ref01_ent:update(card_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local card_ref01_resdata_up0 = helpers.to_map(type(card_ref01_resdata_up0_result) == 'table' and card_ref01_resdata_up0_result.data_get and card_ref01_resdata_up0_result:data_get() or card_ref01_resdata_up0_result)
    assert.is_not_nil(card_ref01_resdata_up0)
    assert.are.equal(card_ref01_resdata_up0["id"], card_ref01_data_up0_up["id"])
    assert.are.equal(card_ref01_resdata_up0[card_ref01_markdef_up0_name], card_ref01_markdef_up0_value)
    ...
    -- REMOVE
    local card_ref01_match_rm0 = {
      id = card_ref01_data["id"],
    }
    local _, err = card_ref01_ent:remove(card_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local card_ref01_match_rt0 = {
      ["list_id"] = setup.idmap["list01"],
    }

    local card_ref01_list_rt0_result, err = card_ref01_ent:list(card_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(card_ref01_list_rt0_result)
...
function card_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "card01", "card02", "card03", "action01", "action02", "action03", "board01", "board02", "board03", "checklist01", "checklist02", "checklist03", "list01", "list02", "list03", "member01", "member02", "member03", "notification01", "notification02", "notification03" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("TRELLO_TEST_CARD_ENTID")
