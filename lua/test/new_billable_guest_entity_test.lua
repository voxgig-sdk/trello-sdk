...
describe("NewBillableGuestEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:NewBillableGuest(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = new_billable_guest_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "new_billable_guest." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_NEW_BILLABLE_GUEST_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local new_billable_guest_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.new_billable_guest")))
    local new_billable_guest_ref01_data = nil
    if #new_billable_guest_ref01_data_raw > 0 then
      new_billable_guest_ref01_data = helpers.to_map(new_billable_guest_ref01_data_raw[1][2])
    end
...
function new_billable_guest_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/new_billable_guest/NewBillableGuestTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read new_billable_guest test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("TRELLO_TEST_NEW_BILLABLE_GUEST_ENTID")
