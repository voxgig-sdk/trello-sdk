...
describe("AttachmentEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_ATTACHMENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local attachment_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.attachment")))
    local attachment_ref01_data = nil
    if #attachment_ref01_data_raw > 0 then
      attachment_ref01_data = helpers.to_map(attachment_ref01_data_raw[1][2])
    end
...
function attachment_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TRELLO_TEST_ATTACHMENT_ENTID")
