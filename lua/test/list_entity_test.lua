...
describe("ListEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_LIST_ENTID JSON to run live")
      return
    end
    ...
    local list_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.list"), "list_ref01"))
    list_ref01_data["board_id"] = setup.idmap["board01"]

    local list_ref01_data_result, err = list_ref01_ent:create(list_ref01_data, nil)
    assert.is_nil(err)
...
function list_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TRELLO_TEST_LIST_ENTID")
