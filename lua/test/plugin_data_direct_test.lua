...
describe("PluginDataDirect", function()
  it("should direct-list-plugin_data", function()
    local setup = plugin_data_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-plugin_data", setup.live and "live" or "unit")
  ...
  it("should direct-load-plugin_data", function()
    local setup = plugin_data_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-plugin_data", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function plugin_data_direct_setup(mockres)
