...
describe("EnterprisSignupUrlEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EnterprisSignupUrl(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = enterpris_signup_url_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "enterpris_signup_url." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_ENTERPRIS_SIGNUP_URL_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local enterpris_signup_url_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.enterpris_signup_url")))
    local enterpris_signup_url_ref01_data = nil
    if #enterpris_signup_url_ref01_data_raw > 0 then
      enterpris_signup_url_ref01_data = helpers.to_map(enterpris_signup_url_ref01_data_raw[1][2])
    end
...
function enterpris_signup_url_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/enterpris_signup_url/EnterprisSignupUrlTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read enterpris_signup_url test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("TRELLO_TEST_ENTERPRIS_SIGNUP_URL_ENTID")
