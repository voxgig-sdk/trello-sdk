...
describe("ReactionEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_REACTION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local reaction_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.reaction")))
    local reaction_ref01_data = nil
    if #reaction_ref01_data_raw > 0 then
      reaction_ref01_data = helpers.to_map(reaction_ref01_data_raw[1][2])
    end
...
function reaction_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TRELLO_TEST_REACTION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["TRELLO_TEST_REACTION_ENTID"])
