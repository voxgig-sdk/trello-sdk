-- NotificationChannelSetting direct test
...
describe("NotificationChannelSettingDirect", function()
  it("should direct-list-notification_channel_setting", function()
    local setup = notification_channel_setting_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-notification_channel_setting", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"member01"}) do
  ...
  it("should direct-load-notification_channel_setting", function()
    local setup = notification_channel_setting_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-notification_channel_setting", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function notification_channel_setting_direct_setup(mockres)
