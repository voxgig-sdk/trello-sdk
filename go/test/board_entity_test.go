...
func TestBoardEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		boardRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "board"}, setup.data), "board_ref01"))
		boardRef01Data["member_id"] = setup.idmap["member01"]
		boardRef01Data["organization_id"] = setup.idmap["organization01"]
		...
		boardRef01MarkdefUp0Name := "creationMethod"
...
func boardBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"board01", "board02", "board03", "action01", "action02", "action03", "card01", "card02", "card03", "checklist01", "checklist02", "checklist03", "list01", "list02", "list03", "member01", "member02", "member03", "notification01", "notification02", "notification03", "organization01", "organization02", "organization03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("TRELLO_TEST_BOARD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRELLO_TEST_BOARD_ENTID"])
