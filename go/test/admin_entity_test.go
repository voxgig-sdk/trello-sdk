...
func TestAdminEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_ADMIN_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		adminRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.admin", setup.data)))
...
func adminBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TRELLO_TEST_ADMIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRELLO_TEST_ADMIN_ENTID"])
