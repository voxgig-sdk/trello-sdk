...
func TestEmojiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_EMOJI_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		emojiRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.emoji", setup.data)))
		...
		_, emojiRef01ListOk := emojiRef01ListResult.([]any)
		if !emojiRef01ListOk {
...
func emojiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TRELLO_TEST_EMOJI_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRELLO_TEST_EMOJI_ENTID"])
