...
func tokenBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"token01", "token02", "token03", "member01", "member02", "member03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("TRELLO_TEST_TOKEN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRELLO_TEST_TOKEN_ENTID"])
