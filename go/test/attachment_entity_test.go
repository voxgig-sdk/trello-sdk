...
func TestAttachmentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		attachmentRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.attachment", setup.data)))
		var attachmentRef01Data map[string]any
		if len(attachmentRef01DataRaw) > 0 {
			attachmentRef01Data = core.ToMapAny(attachmentRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = attachmentRef01Data
		...
		_, attachmentRef01ListOk := attachmentRef01ListResult.([]any)
...
func attachmentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"attachment01", "attachment02", "attachment03", "card01", "card02", "card03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("TRELLO_TEST_ATTACHMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRELLO_TEST_ATTACHMENT_ENTID"])
