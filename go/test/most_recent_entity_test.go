...
func TestMostRecentEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		...
		ent := testsdk.MostRecent(nil)
		if ent == nil {
			t.Fatal("expected non-nil MostRecentEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := most_recentBasicSetup(nil)
		...
		for _, _op := range []string{} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "most_recent." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		_ = mostRecentRef01Data

	})
}

func most_recentBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "most_recent", "MostRecentTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read most_recent test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse most_recent test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("TRELLO_TEST_MOST_RECENT_ENTID")
