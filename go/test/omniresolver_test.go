...
import (
	...
	omni "github.com/voxgig-sdk/trello-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TrelloError); is {
				out := map[string]any{
					"name":    "TrelloError",
...
func omniCtx(arg any, client *sdk.TrelloSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TrelloSDK); is {
			client = live
