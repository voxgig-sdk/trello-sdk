...
func TestTagEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TRELLO_TEST_TAG_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func tagBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"tag01", "tag02", "tag03", "organization01", "organization02", "organization03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("TRELLO_TEST_TAG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRELLO_TEST_TAG_ENTID"])
