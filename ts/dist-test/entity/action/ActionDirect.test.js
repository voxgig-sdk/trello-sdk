...
(0, node_test_1.describe)('ActionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-action', async (t) => {
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.board_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.board_id = candidateId;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['TRELLO_TEST_ACTION_ENTID'];
