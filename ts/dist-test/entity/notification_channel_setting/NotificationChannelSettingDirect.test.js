...
(0, node_test_1.describe)('NotificationChannelSettingDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-notification_channel_setting', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-notification_channel_setting', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["channel01", "member01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.channel ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.channel = candidateId;
    ...
    (0, node_test_1.test)('direct-list-notification_channel_setting', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-notification_channel_setting', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.TrelloSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.TRELLO_APIKEY,
        }));
        let idmap = env['TRELLO_TEST_NOTIFICATION_CHANNEL_SETTING_ENTID'];
    ...
    const client = new __1.TrelloSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=NotificationChannelSettingDirect.test.js.map