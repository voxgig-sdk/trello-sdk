...
(0, node_test_1.describe)('NotificationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-notification', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-notification', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-notification', async (t) => {
        ...
        const result = await client.direct({
            path: 'members/{member_id}/notifications',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['TRELLO_TEST_NOTIFICATION_ENTID'];
