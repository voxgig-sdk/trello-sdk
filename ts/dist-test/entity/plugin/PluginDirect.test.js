...
(0, node_test_1.describe)('PluginDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-plugin', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-plugin', setup.live))
            return;
        ...
        const result = await client.direct({
            path: 'boards/{board_id}/boardPlugins',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['TRELLO_TEST_PLUGIN_ENTID'];
