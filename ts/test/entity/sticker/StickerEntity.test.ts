...
describe('StickerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'sticker.' + op, live)) return
    }
    ...
    let sticker_ref01_data = Object.values(setup.data.existing.sticker)[0] as any

    // UPDATE
    const sticker_ref01_ent = client.Sticker()
    const sticker_ref01_data_up0: any = {}
    ...
    // LOAD
    const sticker_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TRELLO_TEST_STICKER_ENTID']
