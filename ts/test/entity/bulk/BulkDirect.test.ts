...
describe('BulkDirect', async () => {
  ...
  test('direct-load-bulk', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-bulk', setup.live)) return
    if (skipIfMissingIds(t, setup, ["enterpris_id01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_BULK_ENTID']
