...
describe('BulkEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'bulk.' + op, live)) return
    }
    ...
    let bulk_ref01_data = Object.values(setup.data.existing.bulk)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TRELLO_TEST_BULK_ENTID']
