...
describe('SavedSearchDirect', async () => {
  ...
  test('direct-load-saved_search', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-saved_search', setup.live)) return
  ...
  test('direct-list-saved_search', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-saved_search', setup.live)) return
    if (skipIfMissingIds(t, setup, ["member01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_SAVED_SEARCH_ENTID']
