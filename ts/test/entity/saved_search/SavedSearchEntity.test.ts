...
describe('SavedSearchEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load', 'remove']) {
      if (maybeSkipControl(t, 'entityOp', 'saved_search.' + op, live)) return
    }
    ...
    // LIST
    const saved_search_ref01_match: any = {}
    saved_search_ref01_match['member_id'] = setup.idmap['member01']

    const saved_search_ref01_list = (await saved_search_ref01_ent.list(saved_search_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(saved_search_ref01_list, { id: saved_search_ref01_data.id })))


    // UPDATE
    const saved_search_ref01_data_up0: any = {}
    ...
    ;(saved_search_ref01_data_up0 as any)[saved_search_ref01_markdef_up0.name] = saved_search_ref01_markdef_up0.value
    ...
    assert((saved_search_ref01_resdata_up0 as any)[saved_search_ref01_markdef_up0.name] === saved_search_ref01_markdef_up0.value)


    // LOAD
    const saved_search_ref01_match_dt0: any = {}
    ...
    // REMOVE
    const saved_search_ref01_match_rm0: any = { id: saved_search_ref01_data.id }
    await saved_search_ref01_ent.remove(saved_search_ref01_match_rm0)
  

    // LIST
    const saved_search_ref01_match_rt0: any = {}
    saved_search_ref01_match_rt0['member_id'] = setup.idmap['member01']

    const saved_search_ref01_list_rt0 = (await saved_search_ref01_ent.list(saved_search_ref01_match_rt0)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TRELLO_TEST_SAVED_SEARCH_ENTID']
