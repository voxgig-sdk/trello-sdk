...
describe('NotificationChannelSettingDirect', async () => {
  ...
  test('direct-load-notification_channel_setting', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-notification_channel_setting', setup.live)) return
    if (skipIfMissingIds(t, setup, ["channel01","member01"])) return
  ...
  test('direct-list-notification_channel_setting', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-notification_channel_setting', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_NOTIFICATION_CHANNEL_SETTING_ENTID']
