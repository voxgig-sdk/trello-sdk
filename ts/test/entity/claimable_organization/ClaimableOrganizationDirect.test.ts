...
describe('ClaimableOrganizationDirect', async () => {
  ...
  test('direct-list-claimable_organization', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-claimable_organization', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_CLAIMABLE_ORGANIZATION_ENTID']
