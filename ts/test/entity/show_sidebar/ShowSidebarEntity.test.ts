...
describe('ShowSidebarEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update']) {
      if (maybeSkipControl(t, 'entityOp', 'show_sidebar.' + op, live)) return
    }
    ...
    let show_sidebar_ref01_data = Object.values(setup.data.existing.show_sidebar)[0] as any

    // UPDATE
    const show_sidebar_ref01_ent = client.ShowSidebar()
    const show_sidebar_ref01_data_up0: any = {}
