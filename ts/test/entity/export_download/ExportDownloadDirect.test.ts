...
describe('ExportDownloadDirect', async () => {
  ...
  test('direct-load-export_download', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-export_download', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_EXPORT_DOWNLOAD_ENTID']
