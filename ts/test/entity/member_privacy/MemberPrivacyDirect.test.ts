...
describe('MemberPrivacyDirect', async () => {
  ...
  test('direct-load-member_privacy', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-member_privacy', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_MEMBER_PRIVACY_ENTID']
