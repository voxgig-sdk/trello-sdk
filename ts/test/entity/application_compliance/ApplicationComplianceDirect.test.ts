...
describe('ApplicationComplianceDirect', async () => {
  ...
  test('direct-load-application_compliance', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-application_compliance', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_APPLICATION_COMPLIANCE_ENTID']
