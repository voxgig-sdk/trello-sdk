...
describe('NewBillableGuestDirect', async () => {
  ...
  test('direct-load-new_billable_guest', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-new_billable_guest', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TRELLO_TEST_NEW_BILLABLE_GUEST_ENTID']
