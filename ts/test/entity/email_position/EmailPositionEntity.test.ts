...
describe('EmailPositionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update']) {
      if (maybeSkipControl(t, 'entityOp', 'email_position.' + op, live)) return
    }
    ...
    let email_position_ref01_data = Object.values(setup.data.existing.email_position)[0] as any

    // UPDATE
    const email_position_ref01_ent = client.EmailPosition()
    const email_position_ref01_data_up0: any = {}
