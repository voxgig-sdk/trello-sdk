...
describe('ShowSidebarActivityEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update']) {
      if (maybeSkipControl(t, 'entityOp', 'show_sidebar_activity.' + op, live)) return
    }
    ...
    let show_sidebar_activity_ref01_data = Object.values(setup.data.existing.show_sidebar_activity)[0] as any

    // UPDATE
    const show_sidebar_activity_ref01_ent = client.ShowSidebarActivity()
    const show_sidebar_activity_ref01_data_up0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TRELLO_TEST_SHOW_SIDEBAR_ACTIVITY_ENTID']
