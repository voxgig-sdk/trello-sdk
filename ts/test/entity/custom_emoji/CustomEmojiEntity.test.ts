...
describe('CustomEmojiEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'custom_emoji.' + op, live)) return
    }
    ...
    // LIST
    const custom_emoji_ref01_match: any = {}
    custom_emoji_ref01_match['member_id'] = setup.idmap['member01']

    const custom_emoji_ref01_list = (await custom_emoji_ref01_ent.list(custom_emoji_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(custom_emoji_ref01_list, { id: custom_emoji_ref01_data.id })))


    // LOAD
    const custom_emoji_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TRELLO_TEST_CUSTOM_EMOJI_ENTID']
