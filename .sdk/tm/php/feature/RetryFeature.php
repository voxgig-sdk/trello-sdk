...
class TrelloRetryFeature extends TrelloBaseFeature
{
    ...
    public function init(TrelloContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TrelloContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TrelloContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TrelloContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
