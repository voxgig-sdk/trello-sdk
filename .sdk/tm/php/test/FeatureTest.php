...
class FtCtrl extends TrelloControl
{
...
class FtClient
{
    ...
    private TrelloUtility $utility;

    public function __construct(TrelloUtility $utility, array $options)
    {
    ...
    public function get_utility(): TrelloUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TrelloContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TrelloContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TrelloConfig::make_feature($name);
    ...
    private static function buildUrl(TrelloSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TrelloError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TrelloResponse($fetched);
        }
    ...
    private function fail(TrelloContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TrelloContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
