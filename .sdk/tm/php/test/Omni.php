...
//    only - a typed TrelloContext there would make every ctx
...
final class TrelloOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TrelloContext($clean, null);
    ...
    public static function err_from_map($m): ?TrelloError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TrelloError(is_string($code) ? $code : '', $msg);
    }
