...
{
    private static function utility(): TrelloUtility
    {
        return new TrelloUtility();
    }
    ...
    private static function ctx(array $over = []): TrelloContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TrelloContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TrelloOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TrelloError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TrelloOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TrelloMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TrelloOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TrelloMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TrelloOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TrelloMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TrelloSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TrelloMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TrelloResponse([]),
            'result' => new TrelloResult([]),
        ]);
        [, $err] = TrelloMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TrelloSpec(['step' => 's']),
            'response' => new TrelloResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TrelloResult([]),
        ]);
        [, $err] = TrelloMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TrelloSpec(['step' => 's']),
            'response' => new TrelloResponse(self::resp(200, ['v' => 1])),
            'result' => new TrelloResult([]),
        ]);
        [, $err] = TrelloMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TrelloSpec(['step' => 's']),
            'response' => new TrelloResponse(self::resp(200, ['v' => 2])),
            'result' => new TrelloResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TrelloMakeResponse::call($ctx);
    ...
    {
        $preset = new TrelloResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TrelloSpec([]),
            'response' => new TrelloResponse([]),
            'result' => new TrelloResult([]),
        ...
        [$got, $err] = TrelloMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TrelloResult([])]);
        [, $err] = TrelloMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TrelloSpec([]), 'result' => null]);
        [, $err] = TrelloMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TrelloSpec(['step' => 's']),
            'result' => new TrelloResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TrelloOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TrelloMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TrelloSpec(['step' => 's']),
            'result' => new TrelloResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TrelloOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TrelloMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TrelloResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TrelloSpec([]),
            'result' => new TrelloResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TrelloMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TrelloMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TrelloError('boom', 'boom');
        $utility->fetcher = function (TrelloContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TrelloSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TrelloMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TrelloContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TrelloMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TrelloResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TrelloContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TrelloSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TrelloMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TrelloResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TrelloSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TrelloMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TrelloMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TrelloMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TrelloResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TrelloDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TrelloResult(['ok' => false])]);
        $this->expectException(TrelloError::class);
        TrelloDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TrelloResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TrelloMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TrelloBaseFeature();
        $b = new TrelloBaseFeature();
        TrelloFeatureAdd::call($ctx, $a);
        TrelloFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TrelloBaseFeature
    {
        $f = new TrelloBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TrelloFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TrelloContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TrelloSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TrelloPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TrelloResponse(['headers' => null]),
            'result' => new TrelloResult([]),
        ]);
        TrelloResultHeaders::call($ctx);
