...
boru-driven command-line client **and** interactive REPL for the Trello
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/trello-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/trello-cli
   ```
...
Run with no arguments to open a REPL (prompt `trello>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/trello-cli`. |
...
(`./trello-cli <expr>`) and interactively (the REPL), and expressions compose the
