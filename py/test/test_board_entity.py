    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TRELLO_TEST_BOARD_ENTID JSON to run live")
        client = setup["client"]
        ...
        board_ref01_data = helpers.to_map(runner.entity_data(board_ref01_ent.create(board_ref01_data, None)))
        assert board_ref01_data is not None
        assert board_ref01_data["id"] is not None
        ...
        board_ref01_markdef_up0_value = "Mark01-board_ref01_" + str(setup["now"])
        board_ref01_data_up0_up[board_ref01_markdef_up0_name] = board_ref01_markdef_up0_value

        board_ref01_resdata_up0 = helpers.to_map(runner.entity_data(board_ref01_ent.update(board_ref01_data_up0_up, None)))
        assert board_ref01_resdata_up0 is not None
        assert board_ref01_resdata_up0["id"] == board_ref01_data_up0_up["id"]
        assert board_ref01_resdata_up0[board_ref01_markdef_up0_name] == board_ref01_markdef_up0_value

        # LOAD
        board_ref01_match_dt0 = {
            "id": board_ref01_data["id"],
        }
        board_ref01_data_dt0_loaded = board_ref01_ent.load(board_ref01_match_dt0, None)
        board_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(board_ref01_data_dt0_loaded))
        assert board_ref01_data_dt0_load_result is not None
        assert board_ref01_data_dt0_load_result["id"] == board_ref01_data["id"]

        # REMOVE
        board_ref01_match_rm0 = {
            "id": board_ref01_data["id"],
        }
        board_ref01_ent.remove(board_ref01_match_rm0, None)
        ...
        board_ref01_list_rt0_result = board_ref01_ent.list(board_ref01_match_rt0, None)
        assert isinstance(board_ref01_list_rt0_result, list)
...
def _board_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("TRELLO_TEST_BOARD_ENTID"))
