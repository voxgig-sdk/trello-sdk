...
class TestEnterprisSignupUrlEntity:

    def test_should_create_instance(self):
        testsdk = TrelloSDK.test(None, None)
        ent = testsdk.EnterprisSignupUrl(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _enterpris_signup_url_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "enterpris_signup_url." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TRELLO_TEST_ENTERPRIS_SIGNUP_URL_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        enterpris_signup_url_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.enterpris_signup_url")))
        enterpris_signup_url_ref01_data = None
        if len(enterpris_signup_url_ref01_data_raw) > 0:
...
def _enterpris_signup_url_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/enterpris_signup_url/EnterprisSignupUrlTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "TRELLO_TEST_ENTERPRIS_SIGNUP_URL_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TRELLO_TEST_ENTERPRIS_SIGNUP_URL_ENTID"))
