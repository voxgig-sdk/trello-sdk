...
class TestNotificationMemberCreatorEntity:

    def test_should_create_instance(self):
        testsdk = TrelloSDK.test(None, None)
        ent = testsdk.NotificationMemberCreator(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _notification_member_creator_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "notification_member_creator." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TRELLO_TEST_NOTIFICATION_MEMBER_CREATOR_ENTID JSON to run live")
        client = setup["client"]
        ...
        notification_member_creator_ref01_data = None
        if len(notification_member_creator_ref01_data_raw) > 0:
...
def _notification_member_creator_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/notification_member_creator/NotificationMemberCreatorTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "TRELLO_TEST_NOTIFICATION_MEMBER_CREATOR_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TRELLO_TEST_NOTIFICATION_MEMBER_CREATOR_ENTID"))
