...
class TestIdLabelEntity:

    def test_should_create_instance(self):
        testsdk = TrelloSDK.test(None, None)
        ent = testsdk.IdLabel(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _id_label_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "id_label." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TRELLO_TEST_ID_LABEL_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        id_label_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.id_label")))
        id_label_ref01_data = None
        if len(id_label_ref01_data_raw) > 0:
            id_label_ref01_data = helpers.to_map(id_label_ref01_data_raw[0][1])



def _id_label_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/id_label/IdLabelTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TRELLO_TEST_ID_LABEL_ENTID"))
