...
class TestBulkDirect:

    def test_should_direct_load_bulk(self):
        setup = _bulk_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-bulk", "live" if setup["live"] else "unit")
        if _skip:
...
def _bulk_direct_setup(mockres):
