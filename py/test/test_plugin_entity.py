...
class TestPluginEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Plugin(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Plugin(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TRELLO_TEST_PLUGIN_ENTID JSON to run live")
        client = setup["client"]
        ...
        plugin_ref01_list_result = plugin_ref01_ent.list(plugin_ref01_match, None)
        assert isinstance(plugin_ref01_list_result, list)
        ...
        plugin_ref01_resdata_up0 = helpers.to_map(runner.entity_data(plugin_ref01_ent.update(plugin_ref01_data_up0_up, None)))
        assert plugin_ref01_resdata_up0 is not None
        assert plugin_ref01_resdata_up0["id"] == plugin_ref01_data_up0_up["id"]
...
def _plugin_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("TRELLO_TEST_PLUGIN_ENTID"))
