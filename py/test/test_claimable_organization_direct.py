...
class TestClaimableOrganizationDirect:

    def test_should_direct_list_claimable_organization(self):
        setup = _claimable_organization_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-claimable_organization", "live" if setup["live"] else "unit")
        if _skip:
...
def _claimable_organization_direct_setup(mockres):
