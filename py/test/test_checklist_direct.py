...
class TestChecklistDirect:

    def test_should_direct_load_checklist(self):
        setup = _checklist_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-checklist", "live" if setup["live"] else "unit")
        if _skip:
...
def _checklist_direct_setup(mockres):
