...
class TestEmojiDirect:
    ...
    def test_should_direct_list_emoji(self):
        setup = _emoji_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-emoji", "live" if setup["live"] else "unit")
        if _skip:
