...
class TestEnterprisDirect:

    def test_should_direct_load_enterpris(self):
        setup = _enterpris_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-enterpris", "live" if setup["live"] else "unit")
        if _skip:
...
def _enterpris_direct_setup(mockres):
