    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TRELLO_TEST_GENERATE_ENTID JSON to run live")
        client = setup["client"]
        ...
        generate_ref01_data = helpers.to_map(runner.entity_data(generate_ref01_ent.create(generate_ref01_data, None)))
...
def _generate_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("TRELLO_TEST_GENERATE_ENTID"))
