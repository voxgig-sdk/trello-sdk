...
class TestEnterpriseAuditLogDirect:

    def test_should_direct_list_enterprise_audit_log(self):
        setup = _enterprise_audit_log_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-enterprise_audit_log", "live" if setup["live"] else "unit")
        if _skip:
...
def _enterprise_audit_log_direct_setup(mockres):
