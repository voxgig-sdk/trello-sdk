...
class IdLabelEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IdLabelEntity(self._client, opts)
    ...
    def remove(self, reqmatch=None, ctrl=None) -> IdLabel:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.IdLabel().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
