...
class BoardPluginEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BoardPluginEntity(self._client, opts)
    ...
    def match_get(self) -> BoardPlugin:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> BoardPlugin:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.BoardPlugin().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
