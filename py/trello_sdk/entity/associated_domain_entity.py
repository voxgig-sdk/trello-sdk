...
class AssociatedDomainEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AssociatedDomainEntity(self._client, opts)
    ...
    def data_get(self) -> AssociatedDomain:
    ...
    def match_get(self) -> AssociatedDomain:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> AssociatedDomain:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.AssociatedDomain().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
