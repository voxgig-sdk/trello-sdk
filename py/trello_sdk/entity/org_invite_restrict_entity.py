...
class OrgInviteRestrictEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OrgInviteRestrictEntity(self._client, opts)
    ...
    def data_get(self) -> OrgInviteRestrict:
    ...
    def match_get(self) -> OrgInviteRestrict:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> OrgInviteRestrict:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.OrgInviteRestrict().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
