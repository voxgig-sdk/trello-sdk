...
class CustomBoardBackgroundEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CustomBoardBackgroundEntity(self._client, opts)
    ...
    def data_get(self) -> CustomBoardBackground:
    ...
    def match_get(self) -> CustomBoardBackground:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> CustomBoardBackground:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.CustomBoardBackground().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
