...
class NotificationChannelSettingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NotificationChannelSettingEntity(self._client, opts)
    ...
    def data_get(self) -> NotificationChannelSetting:
    ...
    def match_get(self) -> NotificationChannelSetting:
    ...
    def load(self, reqmatch=None, ctrl=None) -> NotificationChannelSetting:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.NotificationChannelSetting().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[NotificationChannelSetting]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.NotificationChannelSetting().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def update(self, reqdata: NotificationChannelSettingUpdateData, ctrl=None) -> NotificationChannelSetting:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
