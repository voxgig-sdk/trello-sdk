...
from trello_sdk.trello_types import (
    BoardBackground,
...
class BoardBackgroundEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BoardBackgroundEntity(self._client, opts)
    ...
    def data_get(self) -> BoardBackground:
    ...
    def match_get(self) -> BoardBackground:
    ...
    def load(self, reqmatch=None, ctrl=None) -> BoardBackground:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.BoardBackground().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[BoardBackground]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.BoardBackground().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: BoardBackgroundCreateData, ctrl=None) -> BoardBackground:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: BoardBackgroundUpdateData, ctrl=None) -> BoardBackground:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> BoardBackground:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.BoardBackground().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
