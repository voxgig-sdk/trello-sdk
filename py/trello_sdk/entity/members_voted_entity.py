...
class MembersVotedEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MembersVotedEntity(self._client, opts)
    ...
    def data_get(self) -> MembersVoted:
    ...
    def match_get(self) -> MembersVoted:
    ...
    def load(self, reqmatch=None, ctrl=None) -> MembersVoted:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.MembersVoted().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> MembersVoted:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.MembersVoted().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
